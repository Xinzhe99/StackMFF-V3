...
# @Description : Network architecture for StackMFF-V3-Star.
...
class StackMFF_V3_Star(nn.Module):
    """StackMFF-V3-Star: Multi-focus image fusion network with continuous depth estimation."""
    def __init__(self, encoder_dims=[32,64,128,256], encoder_layers=[2,2,2,2], num_transformer_layers=2):
        super(StackMFF_V3_Star, self).__init__()
        ...
        # self.layer_interaction = LayerInteraction(encoder_embed_dim)
    ...
    def forward(self, x):
        ...
        # print('respective_features:', respective_features.shape)

        # layer_interaction_features = self.layer_interaction(respective_features)
        layer_interaction_features = self.layer_interaction(respective_features) # Shape: [batch_size, num_images, height, width]

        depth_map = self.depth_map_creation(layer_interaction_features, x.shape[1]) # Shape: [batch_size, 1, height, width]

        fused_image, depth_map_index = self.generate_fused_image(x, depth_map) # Shape: [batch_size, 1, height, width]

        return fused_image, depth_map, depth_map_index
        # return layer_interaction_features
        # return respective_features
...
if __name__ == "__main__":
    ...
    model = StackMFF_V3_Star().to(device)
    ...
    model.eval()
    with torch.no_grad():
        fused_image, depth_map, depth_map_index = model(x)
    
    # Print output shapes
    print(f"Input shape: {x.shape}")
    print(f"Fused image shape: {fused_image.shape}")
    print(f"Depth map shape: {depth_map.shape}")
    print(f"Depth map index shape: {depth_map_index.shape}")